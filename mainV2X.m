...
%% mydata 로드
persistent mydata_loaded
persistent mydata

if isempty(mydata_loaded)
    data = load('/Users/bigmo/OneDrive/바탕 화면/WiLabV2Xsim-main/highway_1000m_time1250_veh200_2025-04-21.mat');
    mydata = data.mydata;
    mydata_loaded = true;
end
...
while timeManagement.timeNow < simParams.simulationTime
    ...
    % =========================================================
    % 1) 먼저 WiLab 기본 position update는 항상 실행
    %    (activeIDs, time 관리, 내부 상태 유지용)
    % =========================================================
    ...
    % =========================================================
    % 2) 그 다음 mydata 위치로 override
    %    (주의: mydata row = 차량 ID 순서라고 가정)
    % =========================================================
    if ~isempty(mydata)

        % 현재 100ms step
        t_now = positionManagement.NposUpdates + 1;
        t_now = max(1, min(size(mydata,3), t_now));

        % mydata에서 위치/속도 추출
        posX_now  = squeeze(mydata(:,2,t_now));
        posY_now  = squeeze(mydata(:,3,t_now));
        speed_now = squeeze(mydata(:,5,t_now));

        % -----------------------------------------------------
        % 위치 덮어쓰기
        % -----------------------------------------------------
        nTraceVeh = length(posX_now);

        % X/YvehicleReal이 차량 ID 인덱스로 접근된다고 가정
        positionManagement.XvehicleReal(1:nTraceVeh) = posX_now;
        positionManagement.YvehicleReal(1:nTraceVeh) = posY_now;

        % 필요하면 추정 위치도 같이 맞춤
        if length(simValues.XvehicleEstimated) >= nTraceVeh
            simValues.XvehicleEstimated(1:nTraceVeh) = posX_now;
            simValues.YvehicleEstimated(1:nTraceVeh) = posY_now;
        end

        % 속도 덮어쓰기 (speed 필드가 있을 때만)
        if isfield(stationManagement,'speed') && length(stationManagement.speed) >= nTraceVeh
            stationManagement.speed(1:nTraceVeh) = speed_now;
        end

        % -----------------------------------------------------
        % 3) 위치 바꿨으니 거리/채널/이웃 다시 계산
        %    mainPositionUpdate 안에서 이미 한 번 계산했지만,
        %    그건 WiLab 기본 위치 기준이므로 다시 해야 함
        % -----------------------------------------------------
        [positionManagement,stationManagement] = computeDistance(simParams,simValues,stationManagement,positionManagement);

        % old/new vehicle index는 여기선 강제로 전부 old vehicle로 간주
        indexOldVehicles = 1:length(stationManagement.activeIDs);
        indexOldVehiclesToOld = indexOldVehicles;

        [dUpdate,sinrManagement.Shadowing_dB,positionManagement.distanceRealOld] = ...
            updateDistanceChangeForShadowing( ...
                positionManagement.distanceReal, ...
                positionManagement.distanceRealOld, ...
                indexOldVehicles, ...
                indexOldVehiclesToOld, ...
                sinrManagement.Shadowing_dB, ...
                phyParams.stdDevShadowLOS_dB);

        [sinrManagement,simValues.Xmap,simValues.Ymap,phyParams.LOS] = ...
            computeChannelGain( ...
                sinrManagement,stationManagement,positionManagement, ...
                phyParams,simParams,dUpdate);

        [positionManagement,stationManagement] = ...
            computeNeighbors(stationManagement,positionManagement,phyParams);

        %fprintf('[DEBUG] step=%d | veh1 pos=(%.2f, %.2f)\n', ...
            %t_now, posX_now(1), posY_now(1));
    end

    % =========================================================
    % 4) 다음 position update 시간 증가
    % =========================================================
...
%             %% =========
    ...
    elseif abs(timeEvent-timeManagement.timeNextPacket(idEvent))<1e-8   % timeEvent == timeManagement.timeNextPacket(idEvent)
        ...
        if stationManagement.vehicleState(idEvent)==constants.V_STATE_LTE_TXRX % is LTE
            ...
            %% ===== RMR packet generation gating =====
            global RMRPolicyGlobal
            global useRMRScheduling

            generatePacket = true;

            if useRMRScheduling && ~isempty(RMRPolicyGlobal)

                stepIdx = round(timeManagement.timeNow / 0.1) + 1;
                maxStep = size(RMRPolicyGlobal.RMRGenerateFlag, 2);
                stepIdx = max(1, min(stepIdx, maxStep));

                if RMRPolicyGlobal.RMRGenerateFlag(idEvent, stepIdx) == 0
            ...
            if ~generatePacket
                ...
                timeManagement.timeNextPacket(idEvent) = ...
                    round(timeManagement.timeNow + generationInterval, 10);
       ...
       %% =====================================================
        ...
        % 기본 CAM interval
        generationInterval = ...
            timeManagement.generationIntervalDeterministicPart(idEvent);
        ...
        % Fixed-size CPM Object RMR에서는 interval을 직접 지정하지 않음.
        % 매 100ms마다 RMRGenerateFlag를 확인하는 방식.
        if useRMRScheduling
            generationInterval = timeManagement.generationIntervalDeterministicPart(idEvent);
        end
